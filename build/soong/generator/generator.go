...
func init() {
	android.RegisterModuleType("pegasus_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pegasusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pegasusExpandVariables(ctx, dep_file)
	...
	cmd := pegasusExpandVariables(ctx, String(g.properties.Cmd))
