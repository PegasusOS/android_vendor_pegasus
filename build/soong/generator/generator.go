...
func init() {
	android.RegisterModuleType("pegasusos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pegasusosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pegasusosExpandVariables(ctx, dep_file)
	...
	cmd := pegasusosExpandVariables(ctx, String(g.properties.Cmd))
